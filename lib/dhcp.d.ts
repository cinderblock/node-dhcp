...
type DhcpFeature = 'hostname' | 'dns' | 'router' | 'netmask' | 'broadcast' | 'leaseTime' | 'renewalTime' | 'rebindingTime';
...
type ConfigValue = string | number | boolean | string[] | number[] | null | ((this: { config: (key: string) => ConfigValue }) => ConfigValue);
 ...
 */
export function createServer(config: ServerConfig): InstanceType<typeof Server>;

/**
 * Creates a new DHCP client
 */
export function createClient(config?: ClientConfig): InstanceType<typeof Client>;

/**
 * Creates a broadcast handler for DHCP
 */
export function createBroadcastHandler(): Server;

/**
 * Adds a new DHCP option
 */
...
declare const dhcp: {
  ...
  DHCPDISCOVER: typeof DHCPDISCOVER,
  DHCPOFFER: typeof DHCPOFFER,
  DHCPREQUEST: typeof DHCPREQUEST,
  DHCPDECLINE: typeof DHCPDECLINE,
  DHCPACK: typeof DHCPACK,
  DHCPNAK: typeof DHCPNAK,
  DHCPRELEASE: typeof DHCPRELEASE,
  DHCPINFORM: typeof DHCPINFORM
};
